...
#     scaler = StandardScaler()
#     pca_data[i] = scaler.fit_transform(patient.T).T
...
for id, _ in enumerate(healthy_mean):
    if id % 2 == 0:
        p2, p3, p4= [], [], []
        p7, = axis[id//2].plot(np.arange(LEN_HEALTHY_ICU), pca_data[id//2],color='green')
        p1, = axis[id//2].plot(np.arange(LEN_HEALTHY_ICU), healthy_mean[id],color='blue')
        axis[id//2].fill_between(np.arange(LEN_HEALTHY_ICU), healthy_mean[id], healthy_mean[id] - sem(healthy[:,id,:], axis = 0, ddof = 0),color='gray', alpha=0.2)
        p5, = axis[id//2].plot(np.arange(LEN_HEALTHY_ICU), healthy_mean[id+1],color='red')
        axis[id//2].fill_between(np.arange(LEN_HEALTHY_ICU), healthy_mean[id+1], healthy_mean[id+1] - sem(healthy[:,id+1,:], axis = 0, ddof = 0),color='gray', alpha=0.2)
        for idx in range(8):
            ...
            if idx == 0:
                ...
                p4.append(tmp)
        l = axis[id//2].legend([p1, p2[0], (p3[0], p3[1], p3[2], p3[3]), (p4[0], p4[1], p4[2]), p5, p7], 
                               [f'Channel {id+1}', 'Physical Trigger', 'Rest Trigger', 'Imagery Trigger', f'Channel {id+2}', "Principal Component"], handler_map={tuple: HandlerTuple(ndivide=None)}, loc='right')
plt.title("Standardize mean hemoglobin response and standard error of the mean. (Healthy controls)")
plt.show()
