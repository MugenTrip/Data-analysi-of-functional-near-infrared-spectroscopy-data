...
figure, axis = plt.subplots(4, 1, figsize=(7, 15))
figure.suptitle("Average timeseries over all participants of the initial session of the healthy controls for each area of interest.")
...
for id, _ in enumerate(healthy_normalized_mean):
    if id % 2 == 0:
        p2, p3, p4= [], [], []
        p6, = axis[id//2].plot(np.arange(1224-306), healthy_normalized_mean[id,306:],color='blue')
        axis[id//2].fill_between(np.arange(1224-306), healthy_normalized_mean[id,306:], healthy_normalized_mean[id,306:] - sem(healthy_normalized[:,id,306:], axis = 0, ddof = 0),color='gray', alpha=0.2)        
        p7, = axis[id//2].plot(np.arange(1224-306), healthy_normalized_mean[id+1,306:],color='red')
        axis[id//2].fill_between(np.arange(1224-306), healthy_normalized_mean[id+1,306:], healthy_normalized_mean[id+1,306:] - sem(healthy_normalized[:,id+1,306:], axis = 0, ddof = 0),color='gray', alpha=0.2)        
        ...
        l = axis[id//2].legend([(p4[0], p4[1], p4[2]), (p3[0], p3[1], p3[2]), p6, p7], 
                               ['Imagery Trigger', 'Rest Trigger', f"Channel {id}", f"Channel {id+1}"], handler_map={tuple: HandlerTuple(ndivide=None)})
